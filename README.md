...
-  **Архитектура Page Object Model**:
    - **Page Objects** инкапсулируют действия и локаторы для страниц (авторизация, работа с таблицей).
    - **Component Objects** инкапсулируют повторно используемые элементы интерфейса (диалог/форма создания и редактирования записи).
- **Негативные проверки**:
...
- **CI в GitHub Actions**:
    - **Push в `dev`** → автоматически запускаются **smoke-тесты** (`@smoke`) в **chromium**.
    - **Pull Request в `main`** → автоматически запускаются **regression-тесты** (`@regression`) в **chromium**.
